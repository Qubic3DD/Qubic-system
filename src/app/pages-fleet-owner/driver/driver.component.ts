...
export class DriverComponentFleet implements OnInit {
  ...
  return this.http.get<any>('http://196.168.8.29:8443/profile/fleet-owners/3/drivers').pipe(
...
fetchDrivers(): void {
  this.isLoading = true;
  this.http.get<any>('http://196.168.8.29:8443/profile/fleet-owners/3/drivers').subscribe({
    next: (response) => {
...
openAddDriverModal(): void {
  ...
  addDriver(): void {
    this.isLoading = true;
    this.http
      .post<any>('http://196.168.8.29:8443/profile/drivers', this.newDriver)
      .subscribe({
        next: () => {
          this.fetchDrivers();
  ...
  deleteDriver(id: number): void {
    if (confirm('Are you sure you want to delete this driver?')) {
      this.isLoading = true;
      this.http
        .delete(`http://196.168.8.29:8443/profile/drivers/${id}`)
        .subscribe({
          next: () => {
            this.fetchDrivers();
            this.isLoading = false;
