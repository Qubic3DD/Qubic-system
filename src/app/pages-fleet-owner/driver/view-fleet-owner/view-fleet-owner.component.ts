...
export class FleetDriverProfileComponent implements OnInit {
  driver: any = null;
  isLoading = true;
...
Math = Math;
  fetchDriver(email: string): void {
    ...
    this.http.get<ApiResponse<DriverProfile>>(`http://196.168.8.29:8443/profile/retrieve/${encodedEmail}`).pipe(
      catchError(error => {
...
fetchRecentActivity(driverId: string): void {
  this.http.get<any[]>(`http://196.168.8.29:8443/activity/${driverId}`)
    .pipe(
      catchError(() => of([]))
    )
    .subscribe(activity => {
