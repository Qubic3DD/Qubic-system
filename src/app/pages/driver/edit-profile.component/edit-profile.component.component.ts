...
export class EditProfileComponentComponent implements OnInit {
  ...
  fetchDriverProfile(email: string): void {
    ...
    this.http
      .get<ApiResponse<DriverProfile>>(
        `http://41.76.110.219:8181/profile/retrieve/${email}`
      )
      .subscribe({
        next: (response) => {
          if (response.data) {
  ...
  viewDocument(username: string, documentPurpose: string): void {
    const url = `http://41.76.110.219:8181/api/v1/files/stream?username=${encodeURIComponent(
      username
  ...
  downloadDocument(username: string, documentPurpose: string): void {
    const url = `http://41.76.110.219:8181/api/v1/files/download?username=${encodeURIComponent(
      username
    )}&documentPurpose=${documentPurpose}`;
  ...
  loadUserDocuments(): void {
    ...
    this.http
      .get<{ data: UserDocument[] }>(
        `http://41.76.110.219:8181/api/v1/files/list?username=${this.driver.username}`
      )
      .subscribe({
        next: (response) => {
  ...
  getDocumentUrl(purpose: string): string {
    ...
    return doc
      ? `http://41.76.110.219:8181/api/v1/files/stream?id=${doc.id}`
      : '';
  }
  ...
  updateDriver(): void {
    ...
    this.isLoading = true;
    this.http
      .post<ApiResponse<DriverProfile>>(
        'http://41.76.110.219:8181/profile/edit',
        this.driver
      )
      .subscribe({
        next: (response) => {
