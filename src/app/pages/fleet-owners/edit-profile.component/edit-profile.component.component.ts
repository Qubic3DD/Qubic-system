...
showSuccessPopup() {
  ...
  fetchDriverProfile(email: string): void {
    this.isLoading = true;
    this.http.get<ApiResponse<DriverProfile>>(`http://196.168.8.29:8443/profile/retrieve/${email}`)
      .subscribe({
        next: (response) => {
          if (response.data) {
            this.driver = {
...
viewDocument(username: string, documentPurpose: string): void {
  const url = `http://196.168.8.29:8443/api/v1/files/stream?username=${encodeURIComponent(username)}&documentPurpose=${documentPurpose}`;
  window.open(url, '_blank');
}

downloadDocument(username: string, documentPurpose: string): void {
  const url = `http://196.168.8.29:8443/api/v1/files/download?username=${encodeURIComponent(username)}&documentPurpose=${documentPurpose}`;
...
//
  loadUserDocuments(): void {
    ...
    this.http.get<{data: UserDocument[]}>(`http://196.168.8.29:8443/api/v1/files/list?username=${this.driver.username}`)
      .subscribe({
        next: (response) => {
...
transportTypes = [
  ...
  uploadProfilePicture(file: File): void {
    ...
    this.http.post<{data: {downloadUrl: string}}>(
      `http://196.168.8.29:8443/api/v1/files?userName=${this.driver.username}&fileType=IMAGE&documentPurpose=PROFILE_PICTURE`,
      formData
    ).subscribe({
      next: (response) => {
  ...
  uploadDocument(): void {
    ...
    this.http.post<any>(
      `http://196.168.8.29:8443/api/v1/files?userName=${this.driver.username}&fileType=${this.selectedDocumentType}&documentPurpose=${this.selectedDocumentPurpose}`,
      formData
    ).subscribe({
      next: () => {
        this.loadUserDocuments();
...
getDocumentUrlByUsernameAndPurpose(username: string, purpose: string): string {
  ...
  getDocumentUrl(purpose: string): string {
    ...
    return doc ? `http://196.168.8.29:8443/api/v1/files/stream?id=${doc.id}` : '';
  }
...
updateDriver(): void {
  ...
  this.isLoading = true;
  this.http.post<ApiResponse<DriverProfile>>('http://196.168.8.29:8443/profile/edit', this.driver)
    .subscribe({
      next: (response) => {
        this.isLoading = false;
